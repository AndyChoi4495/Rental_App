...
const LoginScreen = ({ navigation }) => {
    // form fields
    const [emailFromUI, setEmailFromUI] = useState('yun@gmail.com');
    ...
    const verifyOwerAccount = () => {
        for (let i = 0; i < OwnerId.length; i++) {
            if (ownerId[i] === auth.currentUser) return;
            else {
                throw new Error('It is not an owner account');
            }
        }
    };

    const loginPressed = async () => {
        if (auth.currentUser === null && verifyOwerAccount) {
